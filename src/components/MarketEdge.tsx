  ...
  return (
    <section id="mercado" className="mx-auto max-w-[1280px] scroll-mt-24 px-4 pt-12 pb-8 sm:px-6 sm:pt-20 sm:pb-10">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-fg-2">
            模拟概率与去水分后的市场隐含概率对比：
            <span className="text-fg-1">Polymarket</span> / <span className="text-fg-1">Kalshi</span>。
            每行一支球队。偏差为正 = 模型认为被低估。
            <strong className="text-rose"> 不构成金融建议。</strong>
          </p>
        </div>
        <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
          {loading ? '加载中…' : sources.length ? sources.join(' · ') : '无可用数据源'}
        </div>
      </header>
      ...
      {loading ? (
        ...
        <EmptyState message={err || '暂无赔率数据。请稍后刷新 - 缓存 5 分钟。'} />
      ) : (
          ...
          <table className="min-w-full">
            <thead>
              <tr className="text-[10px] uppercase tracking-[0.18em] text-fg-3 font-mono">
                ...
                <th className="px-3 py-3 text-right">模拟</th>
                ...
                <th className="px-3 py-3 text-right border-l border-border/40">最优</th>
              </tr>
              <tr className="text-[10px] uppercase tracking-[0.14em] text-fg-3 font-mono">
                <th className="px-4 py-1.5 text-left" />
                <th className="px-3 py-1.5 text-right" />
                <th className="px-3 py-1.5 text-right border-l border-border/40">概率</th>
                <th className="px-3 py-1.5 text-right">偏差</th>
                <th className="px-3 py-1.5 text-right">期望值</th>
                <th className="px-3 py-1.5 text-right border-l border-border/40">概率</th>
                <th className="px-3 py-1.5 text-right">偏差</th>
                <th className="px-3 py-1.5 text-right">期望值</th>
      ...
      <p className="mt-4 text-[10px] leading-relaxed text-fg-3">
        注意：偏差为正不保证盈利。模型基于历史 ELO 评分，不包含伤病、近期状态、教练变动等因素。
        各数据源独立去水（Polymarket 二元市场、Kalshi 买卖中间价）。此分析仅供研究参考，
        不构成金融建议。请理性判断。
      </p>
    </section>
  );
