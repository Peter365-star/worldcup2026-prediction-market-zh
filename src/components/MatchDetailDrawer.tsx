          ...
          {stats && fixture && (
            <>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8">
                ...
                {/* W/D/L */}
                <section>
                  <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">胜负概率</h3>
                  <div className="grid grid-cols-3 gap-3">
                    <Probability label={locale === 'es' ? stats.homeTeam.name_es : stats.homeTeam.name_en} value={stats.winsHome} tone="emerald" />
                    <Probability label="平局" value={stats.draws} tone="muted" />
                ...
                <section>
                  <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                    其他 <span className="text-fg-3">· 简易估算</span>
                  </h3>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <StatLine label="黄牌（主队）"  value={stats.yellowsHome.toFixed(1)} />
                    <StatLine label="黄牌（客队）" value={stats.yellowsAway.toFixed(1)} />
                ...
                <p className="text-[10px] text-fg-3 leading-relaxed">
                  胜负概率 calculadas con el modelo ELO + Poisson sobre {result.numSimulations.toLocaleString()} simulaciones.
...
function ScorersList({ teamId, lambda }: { teamId: string; lambda: number }) {
  const scorers = getScorers(teamId).filter((s) => s.name !== '其他');
  return (
