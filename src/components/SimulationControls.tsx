...
export function SimulationControls({ state, onRun }: Props) {
  ...
  const effectiveN = useCustom && customVal ? Math.max(1, Math.min(1_000_000, Number(customVal) || 10_000)) : selected;
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:gap-5">
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {OPTIONS.map((n) => {
            const active = !useCustom && selected === n;
            return (
              <button
                key={n}
                disabled={isRunning}
          ...
          <input
            ...
            placeholder="自定义"
            className={cn(
              'w-24 rounded-full border bg-bg-1/30 px-3 py-1.5 font-mono text-xs tracking-wider text-fg-1 outline-none transition-all placeholder:text-fg-3',
              useCustom
                ? 'border-gold/40 bg-bg-1/50'
