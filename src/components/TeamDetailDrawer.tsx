...
export function TeamDetailDrawer({ result, resultNoAbsences }: Props) {
  ...
  const data = useMemo(() => {
    ...
    let noAbs: { wasChamp: number; outInGroups: number; reach决赛: number; reachSF: number } | null = null;
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8">
                ...
                <section className="grid grid-cols-2 gap-3">
                  <BestWorstCard
                    tone="gold" label="Probabilidad campeón" caption="最佳情况"
                    value={data.wasChamp}
                    ci={wilsonCI(result.stageCounts.champion[teamIdx], result.numSimulations)}
                  />
                  <BestWorstCard
                    tone="rose" label="小组出局" caption="最差情况"
                ...
                <section>
                  ...
                  <div className="space-y-1.5">
                    <StageBar label="32强" value={data.reachR32}  count={result.stageCounts.r32[teamIdx]}      n={result.numSimulations} />
                    <StageBar label="16强"   value={data.reachR16}  count={result.stageCounts.r16[teamIdx]}      n={result.numSimulations} />
                    <StageBar label="八强"        value={data.reachQF}   count={result.stageCounts.qf[teamIdx]}       n={result.numSimulations} />
                    <StageBar label="四强"      value={data.reachSF}   count={result.stageCounts.sf[teamIdx]}       n={result.numSimulations} />
                    <StageBar label="决赛"          value={data.reach决赛} count={result.stageCounts.final[teamIdx]}    n={result.numSimulations} />
                ...
                <section>
                  ...
                  <div className="grid grid-cols-5 gap-2 text-center">
                    <GroupCell label="第1"        value={data.groupFirst}    tone="emerald" />
                    <GroupCell label="第2"        value={data.groupSecond}   tone="emerald" />
                    <GroupCell label="第3(晋级)" value={data.groupThirdAdv} tone="amber" />
                    <GroupCell label="第3(出局)" value={data.groupThirdOut} tone="muted" />
                    <GroupCell label="第4"        value={data.groupFourth}   tone="muted" />
                  </div>
                </section>

                {/* Goals */}
                <section className="grid grid-cols-2 gap-3">
                  <StatBlock label="预期进球" value={data.avgGF.toFixed(2)} sub="promedio por torneo" tone="emerald" />
                  <StatBlock label="预期失球" value={data.avgGA.toFixed(2)} sub="promedio por torneo" tone="rose" />
                </section>
                ...
                {data.noAbs && (
                  <section>
                    <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                      无伤病场景{' '}
                      <span className="text-fg-3">· 假设所有伤病球员可出场</span>
                    </h3>
                    <div className="grid grid-cols-2 gap-3 text-center">
                      ...
                      <DeltaCard
                        label="晋级决赛"
                        current={data.reach决赛}
                        alt={data.noAbs.reach决赛}
                        tone="emerald"
                      />
                      <DeltaCard
                        label="晋级四强"
                ...
                {data.topScorers.length > 0 && (
                  <section>
                    <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                      预期射手 <span className="text-fg-3">· aproximación</span>
                    </h3>
