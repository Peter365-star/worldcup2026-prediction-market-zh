...
export function getCachedMarkets(result: SerializedResult, locale: string): DemoMarket[] {
  const loc = safeLocale(locale);
  if (cache?.result !== result) warmDemoCache(result);
  return cache!.markets[loc];
}

export function getCachedListings(result: SerializedResult, locale: string): DemoTicketListing[] {
  const loc = safeLocale(locale);
  if (cache?.result !== result) warmDemoCache(result);
  return cache!.listings[loc];
}
